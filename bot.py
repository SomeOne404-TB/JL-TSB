...
# 🔧 تعديل مسار قاعدة البيانات للعمل على Railway
def get_db_path():
    # Railway يوفر مجلد /tmp دائم
...
def add_owner_to_approved():
    ...
    try:
        ...
        conn.commit()
        print("✅ Owner added to approved users")
...
# 🔧 دالة محسنة لبدء البوت مع معالجة الأخطاء
async def main_async():
    ...
    application.add_handler(CallbackQueryHandler(handle_button, pattern="^(add_data|view_requests|manage_users|search_data|db_stats|approve_|reject_|ban_|unban_|view_banned_|back_to_main)"))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
    
    # بدء البوت
    print("=" * 50)
    print("🤖 Telegram User Data Bot")
...
def main():
    ...
    try:
        ...
        print(f"❌ Error: {e}")
        # إعادة التشغيل بعد 5 ثواني في حالة الخطأ
